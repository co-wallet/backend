...
func (f *importFixture) configured(t *testing.T) model.ImportPreview {
	...
	require.NoError(t, err)
	p, err = f.svc.Configure(context.Background(), f.user, p.ID, map[string]model.AccountKind{"cash": "spending", "travel": "deposit", "reserve": "investment"})
	require.NoError(t, err)
...
func TestMonefyExclusionsAndStaleCatalog(t *testing.T) {
	...
	p, err = f.svc.Configure(ctx, f.user, p.ID, map[string]model.AccountKind{"cash": "spending", "travel": "deposit", "reserve": "investment"})
	require.NoError(t, err)
