...
func TestExternalTransfers(t *testing.T) {
	...
	require.Equal(t, src.OwnerID, created.Account.OwnerID)
	require.NotNil(t, created.AccountTo)
	require.Equal(t, dst.ID, created.AccountTo.ID)
	require.Equal(t, dst.Currency, created.AccountTo.Currency)
	require.True(t, created.AccountTo.AcceptTransfers)
	...
	require.Equal(t, "Destination", incoming.AccountTo.Name)
	...
	require.Equal(t, src.ID, listed[0].Account.ID)
	require.NotNil(t, listed[0].AccountTo)
	require.Equal(t, "EUR", listed[0].AccountTo.Currency)
	...
	require.NoError(t, err)
	require.NotNil(t, historical.AccountTo)
	require.Equal(t, "Destination", historical.AccountTo.Name)
	require.NotNil(t, historical.AccountTo.DeletedAt)
	historicalList, err := txRepo.List(ctx, sender, model.TransactionFilter{})
	require.NoError(t, err)
	require.Len(t, historicalList, 1)
	require.Equal(t, dst.ID, historicalList[0].AccountTo.ID)
	_, err = svc.Create(ctx, sender, req)
